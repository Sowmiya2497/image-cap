...
from weights import calculateWeights, calcSigma, returnWeights,getWeightsMethod1,weightCalculationMethodSec
...
def preProBuildWordVocab(sentence_iterator, word_count_threshold):
  """Builds vocabulary of words and creates a word-index mapping"""
...
def printWordEmbedding(batch,wordtoix):
  """Function to print embedding of a caption - word to index mapping"""
  for pair in batch:
...
def RNNGenCost(batch, model, params, misc,weightCalculatedData):
  ...
  for i,pair in enumerate(batch):
    ...
    #logppl += - np.sum(np.multiply(wordWeights,np.log2(1e-20 + P[range(len(gtix)),gtix]))) 
    #logppln += len(gtix)
...
def main(params):
  ...
  weightComputedData = getWeightsMethod1()
  print 'Done:'
